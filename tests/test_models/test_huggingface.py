...
class TestHuggingfaceSubject(unittest.TestCase):
    ...
    #     reading_time = model.digest_text(text)['behavior'].values
    #     assert reading_time == 350  # todo

    # TODO: make sure behavioral (next-word, reading-times) and neural readouts work for more than one str input, e.g.
    # def test_behavior_multiple_texts(self):
    ...
    #         next_words = model.digest_text(text)['behavior'].values
    #         np.testing.assert_array_equal(next_words, ['jumps', 'the', 'dog'])  # or whatever the model actually predicts
    #         # similarly, for task=ArtificialSubject.Task.reading_times, outputs would be e.g. [350, 313, 380]
    #
