...
class _Cacheable(typing.Protocol):
    ...
    def to_cache(self, identifier_string: str = None, overwrite = True,
                 cache_dir = None, xarray_serialization_backend='to_zarr') -> Path:
        '''
        ...
        '''
        if cache_dir:
            cache = get_cache_directory(cache_dir, calling_class=self.__class__.__name__)
        else:
        ...
        for ob_name in self._get_xarray_objects():
            ...
            dump_object_fn = getattr(ob.to_dataset(name='data'), xarray_serialization_backend)
        ...
        for key, ob in vars(self).items():
            if isinstance(ob, _Cacheable):
                dest = ob.to_cache(identifier_string=identifier_string, overwrite=overwrite,
                                   xarray_serialization_backend=xarray_serialization_backend,
                                   cache_dir=cache_dir)
                cacheable_ptrs[key] = dest
    ...
    def load_cache(self, identifier_string: str = None, overwrite: bool = True,
                   xarray_deserialization_backend='open_zarr', cache_dir = None) -> Path:
        ...
        with (subdir / 'id.txt').open('r') as f:
            if (identifier_string or self.identifier_string) != (cached_identifier_str := f.read()):
                if not overwrite:
                    raise ValueError(f'mismatch in identifier string of self ({self.identifier_string}) and '
                                     f'cached object ({cached_identifier_str}); overwriting is disabled.')
                else:
                    log(f'mismatch in identifier string of self ({self.identifier_string}) and '
        ...
        for key, ptr in cacheable_ptrs.items():
            try:
                ob = getattr(self, key)
                ob.load_cache(identifier_string=identifier_string, overwrite=overwrite,
                              xarray_deserialization_backend=xarray_deserialization_backend,
                              cache_dir=cache_dir)
            except AttributeError:
                log(f'`load_cache` currently only supports loading xarray objects or initialized `_Cacheable` objects')
    ...
    @classmethod
    def from_cache(cls: typing.Callable[..., T], identifier_string: str, 
        ...
        duck = Duck()
        duck.load_cache(identifier_string, 
                        overwrite=True,
                        xarray_deserialization_backend=xarray_deserialization_backend, 
                        cache_dir=cache_dir)
        return duck
