...
from brainio.assemblies import (
    DataAssembly,
    array_is_element,
    walk_coords,
    merge_data_arrays,
...
def upload_ceiling(experiment):
    benchmark = load_benchmark(f"Pereira2018_v2022.{experiment}-linreg_pearsonr")
    ...
    upload_data_assembly(
        ceiling, assembly_identifier=benchmark.identifier, assembly_prefix="ceiling_"
    )
...
class HoldoutSubjectCeiling:
    ...
    def __call__(self, assembly, metric):
        ...
        scores = []
        iterate_subjects = self._rng.choice(
            list(subjects), size=self._num_bootstraps
        ...
        for subject in tqdm(iterate_subjects, desc="heldout subject"):
            try:
                ...
                # store scores
                apply_raw = "raw" in score.attrs and not hasattr(
                    score.raw, self.subject_column
                )  # only propagate if column not part of score
...
class ExtrapolationCeiling:
    def __init__(
        ...
        extrapolation_dimension="neuroid",
        num_bootstraps=100,
    ):
        self._logger = logging.getLogger(fullname(self))
        self.subject_column = subject_column
    ...
    def __call__(self, assembly, metric):
        scores = self.collect(
            identifier=assembly.identifier, assembly=assembly, metric=metric
        )
    ...
    def collect(self, identifier, assembly, metric):
        ...
        for num_subjects in tqdm(subject_subsamples, desc="num subjects"):
            for sub_subjects in self._random_combinations(
                ...
                choice=self.num_subsamples,
                rng=self._rng,
            ):
                sub_assembly = assembly[
                    {
                        "neuroid": [
                            subject in sub_subjects
                            for subject in assembly[self.subject_column].values
                        ]
                    }
                ]
    ...
    def extrapolate(self, identifier, ceilings):
        ...
        for i in trange(
            len(ceilings[self.extrapolation_dimension]),
            ...
            try:
                ...
                extrapolated_ceiling = self.extrapolate_neuroid(
                    neuroid_ceiling.squeeze()
                )
                extrapolated_ceiling = self.add_neuroid_meta(
                    extrapolated_ceiling, neuroid_ceiling
                )
                ...
                _logger.warning(
                    f"Failed to extrapolate neuroid ceiling {i}", exc_info=True
                )
                continue

        # merge and add meta
        self._logger.debug("Merging neuroid ceilings")
        neuroid_ceilings = manual_merge(
            *neuroid_ceilings, on=self.extrapolation_dimension
        )
    ...
    def extrapolate_neuroid(self, ceilings):
        ...
        for bootstrap in range(self.num_bootstraps):
            ...
            for num_subjects in subject_subsamples:
                ...
                assert set(num_scores.dims) == {
                    f"sub_{self.subject_column}",
                    "split",
                } or set(num_scores.dims) == {f"sub_{self.subject_column}"}
                # choose from subject subsets and the splits therein, with replacement for variance
                choices = num_scores.values.flatten()
                bootstrapped_score = rng.choice(
                    choices, size=len(choices), replace=True
                )
        ...
        end_x = np.where(diffs < asymptote_threshold)[
            0
        ].min()  # first x where increase smaller than threshold
        # put together
...
def manual_merge(*elements, on="neuroid"):
    ...
    merged_values = np.concatenate(
        [element.values for element in elements], axis=merge_index
    )
    ...
    result = type(elements[0])(
        ...
        coords={
            ...
            **{
                coord: (
                    dims,
                    np.concatenate([element[coord].values for element in elements]),
                )
                for coord, dims, _ in walk_coords(elements[0])
                if array_is_element(dims, on)
            },
        },
        dims=elements[0].dims,
    )
...
def _coords_match(elements, dim, match_values=False):
    first_coords = [
        (key, tuple(value)) if match_values else key
...
def ci_error(samples, center, confidence=0.95):
    low, high = 100 * ((1 - confidence) / 2), 100 * (1 - ((1 - confidence) / 2))
    confidence_below, confidence_above = np.nanpercentile(
        samples, low
    ), np.nanpercentile(samples, high)
    confidence_below, confidence_above = (
        center - confidence_below,
        confidence_above - center,
    )
...
if __name__ == "__main__":
    ...
    warnings.filterwarnings(
        "ignore"
    )  # suppress RuntimeWarning from extrapolation overflows
