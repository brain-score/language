...
class HuggingfaceSubject(ArtificialSubject):
    def __init__(
        ...
        self.basemodel = (
            ...
            else AutoModelForCausalLM.from_pretrained(self.model_id)
        )
        self.basemodel.to("cuda" if torch.cuda.is_available() else "cpu")
        self.tokenizer = (
            tokenizer
            if tokenizer is not None
            else AutoTokenizer.from_pretrained(self.model_id, truncation_side="left")
        )
    ...
    def digest_text(self, text: Union[str, List[str]]) -> Dict[str, DataAssembly]:
        ...
        text_iterator = (
            tqdm(text, desc="digest text") if len(text) > 100 else text
        )  # show progress bar if many parts
    ...
    def _tokenize(self, context, num_previous_context_tokens):
        ...
        overflowing_encoding: list = (
            np.array(context_tokens.encodings).item().overflowing
        )
        num_overflowing = (
            0
            if not overflowing_encoding
            else sum(len(overflow) for overflow in overflowing_encoding)
        )
        self.current_tokens = {
            key: value[..., num_previous_context_tokens - num_overflowing :]
    ...
    def _setup_hooks(self):
        ...
        for (recording_target, recording_type) in self.neural_recordings:
            ...
            hook = self._register_hook(
                layer,
                key=(recording_target, recording_type, layer_name),
    ...
    def output_to_representations(
        self,
        layer_representations: Dict[Tuple[str, str, str], np.ndarray],
        stimuli_coords,
    ):
    ...
    def estimate_reading_times(self, base_output: CausalLMOutput):
        ...
        if (
            logits.shape[0] <= 1
        ):  # sequence_length indicates we have only seen single word
        ...
        predicted_logits = logits[
            -self.current_tokens["input_ids"].shape[-1] - 1 : -1, :
        ].contiguous()
        ...
        cross_entropy = F.cross_entropy(
            predicted_logits, actual_tokens, reduction="sum"
        ) / np.log(2)
    ...
    def predict_next_word(self, base_output: CausalLMOutput):
        ...
        last_model_token_inference = (
            pred_id[-1].tolist() if len(pred_id.size()) > 0 else pred_id.item()
        )
    ...
    def _register_hook(
        self, layer: torch.nn.Module, key: Tuple[str, str, str], target_dict: dict
    ) -> RemovableHandle:
        # instantiate parameters to function defaults; otherwise they would change on next function call
        def hook_function(
            _layer: torch.nn.Module, _input, output: torch.Tensor, key=key
        ):
...
class BidirectionalHuggingfaceSubject(HuggingfaceSubject):
    def __init__(
        self,
        model_id: str,
        region_layer_mapping: dict,
        ...
        self.basemodel = (
            model
            if model is not None
            else AutoModelForMaskedLM.from_pretrained(self.model_id)
        )
        self.basemodel.to("cuda" if torch.cuda.is_available() else "cpu")
        # TODO: the below line (left truncation) would be problematic if we have a really really long sentence
        # and in that case we would have to accommodate it using, say, sliding window tokenization over the
        # really long input
        self.tokenizer = (
            tokenizer
            if tokenizer is not None
    ...
    def digest_text(self, text: Union[str, List[str]]) -> Dict[str, DataAssembly]:
        ...
        for part_number, text_part in enumerate(text_iterator):
            # prepare string representation of context
            pre_context = prepare_context(text[:part_number])
            pre_words = pre_context.split(" ")
            starting_word_ix = len(pre_words if pre_words != [""] else [])

            pre_context_plus_current = pre_context + text_part
            ending_word_ix = len(pre_context_plus_current.split(" "))

            context = prepare_context(text[: part_number + 1])
            context_tokens, number_of_tokens = self._tokenize(context, number_of_tokens)

            start_token_ix = context_tokens.word_to_tokens(starting_word_ix)
            end_token_ix = context_tokens.word_to_tokens(ending_word_ix)

            mask_token_id = self.tokenizer.mask_token_id
            mask_tokens = torch.tensor(
                [mask_token_id] * (end_token_ix - start_token_ix)
            )
            context_tokens.input_ids[0, start_token_ix : end_token_ix + 1] = mask_tokens
