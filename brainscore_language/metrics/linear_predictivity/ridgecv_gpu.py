...
if __name__ == "__main__":
    ...
    X_np, y_np = make_regression(n_samples=10000, n_features=20000, noise=0.1, random_state=42)
    alphas = [0.1, 1.0, 10.0]
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print("Testing RidgeGCVTorch against sklearn RidgeCV")

    start_time_gpu = time.time()
    model_torch = RidgeGCVTorch(alphas=alphas, store_cv_results=True, device=device).fit(X_np, y_np)
    end_time_gpu = time.time()
    print(f"Torch RidgeGCVTorch fit time (GPU): {end_time_gpu - start_time_gpu:.4f} seconds")

    
    start_time_cpu = time.time()
    model_skl = RidgeCV(alphas=alphas, store_cv_results=True).fit(X_np, y_np)
    end_time_cpu = time.time()
    print(f"Sklearn RidgeCV fit time (CPU): {end_time_cpu - start_time_cpu:.4f} seconds")
