...
class LM1B(ArtificialSubject):
    ...
    def digest_text(self, text: Union[str, List[str]]) -> Dict[str, DataAssembly]:
        ...
        text_iterator = tqdm(text_parts, desc='digest text') if len(text_parts) > 1 else text_parts  # show progress bar if multiple parts
        logits = []
        for part_number, text_part in enumerate(text_iterator):
            ...
            for i in range(len(word_ids)):
                ...
                fetches = (
                    ...
                    *(self.encoder_t['lstm/%s/control_dependency' 
    ...
    def output_to_representations(self, layer_representations, stimuli_coords):
        ...
        representation_values = np.concatenate(list(layer_representations.values()), axis=-1)  # concatenate along neuron axis
        neuroid_coords = {
    ...
    def _load_encoder(self):   
        ...
        # Downloading resources (checkpoint) if not available
        cur_dirname = os.path.dirname(os.path.realpath(__file__))
        resources_dir = os.path.join(cur_dirname, "resources")
        if not os.path.isdir(resources_dir):
            os.mkdir(resources_dir)
            downloading_iter = tqdm(RESOURCES, desc='Downloading resources for LM1B.')
            for res in downloading_iter:
                file_name = res.split("/")[-1]
                file_path = os.path.join(resources_dir, file_name)
                urllib.request.urlretrieve(res, file_path)

        gd_file = os.path.join(resources_dir, "graph-2016-09-10.pbtxt")
        ckpt_file = os.path.join(resources_dir, "ckpt-*")
        vocab_file = os.path.join(resources_dir, "vocab-2016-09-10.txt")
...
model_registry['lm1b'] = lambda: LM1B(
    ...
    region_layer_mapping={
        ArtificialSubject.RecordingTarget.language_system: 'lstm_1'
    },
)